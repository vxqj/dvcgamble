...
// Same as fetchMe, but also returns isAdmin and bonusTitles — server-
// verified, same as the username. Use this instead of fetchMe wherever the
// app needs to decide whether to show the hidden admin button, or which
// Daily-Reward-granted titles (see DAILY_REWARDS_CONFIG in lib/config.js)
// a player has unlocked. Nothing about either decision ever comes from
// localStorage or anything the client itself computed.
export async function fetchMeFull(token) {
  ...
  try {
    ...
    return { username: data.username, isAdmin: !!data.isAdmin, bonusTitles: data.bonusTitles || [] };
  } catch (e) {
    return null;
  }
}
...
// ----------------------------------------------------------------------
// Daily Rewards — see components/DailyRewardsModal.jsx and
// DAILY_REWARDS_CONFIG in lib/config.js. Coins granted by a "coins" day
// flow through the existing pending_coins column, so they're picked up by
// claimPendingCoins above automatically — the two functions below only
// cover checking streak status, claiming today's reward, and picking up a
// pending "card" grant (there's no equivalent for that already).
// ----------------------------------------------------------------------

export async function fetchDailyStatus(token) {
  if (!token) return null;
  try {
...
export async function claimDailyReward(token) {
  return postJson("/api/daily/claim", {}, token);
}

// Returns { rarityKey, cardName } | null, and clears it server-side —
// same consume-once pattern as consumeForcedPull above.
export async function claimPendingDailyCard(token) {
  if (!token) return null;
  try {
    const res = await fetch("/api/game/claim-daily-card", {
      method: "POST",
      headers: { Authorization: `Bearer ${token}` },
    });
    if (!res.ok) return null;

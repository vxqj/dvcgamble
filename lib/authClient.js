...
//
// Normalizes `bids` to always be an array here, at the one place every
// consumer goes through — a brand-new auction with zero bids can come
// back from the server as `bids: null` (e.g. a JOIN with no matching
// rows), and AuctionTab/AuctionDetailModal both call .length/.find/.map
// on it directly with no guard. Without this, listing a fresh auction —
// the exact case with 0 bids — throws during render and takes down the
// whole auction list with an uncaught error, instead of just showing an
// empty bid history like it should.
export async function fetchAuctions() {
  try {
    const res = await fetch("/api/auction/list");
    if (!res.ok) return [];
    const data = await res.json();
    return (data.auctions || []).map((a) => ({ ...a, bids: a.bids || [] }));
  } catch (e) {
    return [];
  }
}

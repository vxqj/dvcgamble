...
export const RARITIES = [
  ...
  prismatic: true, fx: 8, sellValue: 450000000,
  ...
  prismatic: true, fx: 8, sellValue: 350000000,
  ...
  prismatic: true, fx: 8, sellValue: 150000000,
  ...
  prismatic: true, fx: 7, sellValue: 80000000,
...
export const CARDS = {
  ...
  Transcendent: ["MUD (SR)", "Kayla Beard"],
  ...
  Ascended: ["Rabbi Wilson", "Buddhist Burgess", "Bloody Willis", "Rory", "j2trappy"],
...
export const PACKS = [
  ...
  },
  {
    ...
    cost: 500000,
    cardsPerPack: 5,
    rarityMultipliers: { common: 0.2, rare: 0.6, epic: 2, legendary: 5, mythical: 7, secret: 10, Universal: 7, VM: 20, Ascended: 23, Sovereign: 26, Bloodline: 29, Apex: 31, Sacred: 34, Transcendent: 37, Singularity: 40, Digital: 43, Chaos: 45, Revenant: 49 },
...
export const UPGRADES = {
  multiOpen: {
    ...
    baseCost: 300,
    costGrowth: 2.2,
    maxLevel: 20, // level 0 = 1 pack at once, level 8 = 9 packs at once
  },
  coinBoost: {
    key: "coinBoost",
    label: "Coin Boost",
...
export const EVENT_CONFIG = {
  enabled: true,
  endsAt: "2026-08-04T10:00:00+10:00",
};
...
/* --------------------------------------------------------------------------
   ...
   NEW: DAILY-REWARD-GRANTED TITLES (`bonusOnly: true`)
   A title can also be unlocked purely by being granted through the Daily
   Rewards system (see DAILY_REWARDS_CONFIG below) instead of by rarity,
   admin status, or username. Give it `bonusOnly: true` plus its own
   label/color/gradient, and set that same `key` as a `titleKey` on a
   `type: "title"` day in DAILY_REWARDS_CONFIG. lib/engine.js's
   unlockedTitleKeys checks a player's granted bonus titles against this
   automatically — nothing else needs to change to add another one.
   -------------------------------------------------------------------------- */
export const TITLES = [
  ...
  {
  key: "╭ᑎ╮",
  allowedUsernames: ["FrozenRanger2383"],
  label: "╭ᑎ╮",
  description: "FeetFucker - Tyson Landy",
  color: "#E88E5A",
  gradient: "linear-gradient(120deg, #1a0803 0%, #6b2d14 20%, #E88E5A 50%, #ffb36b 75%, #000000 100%)"
},
{
  key: "✡",
  allowedUsernames: ["NeonTitan2923", "NeonYeti2768", "LuckyCyclone778", "MightyRocket3726", "SilentRocket6866"],
  label: "✡",
  description: "-",
  color: "#005EB8",
  gradient: "linear-gradient(120deg, #ffffff 0%, #eaf4ff 20%, #005EB8 50%, #4da3ff 75%, #ffffff 100%)"
},
{
  key: "danahay",
  ...
  // Example: a title granted only via Daily Rewards Day 2 (see
  // DAILY_REWARDS_CONFIG below). Rename the key/label/colors however you
  // like — just make sure DAILY_REWARDS_CONFIG's Day 2 titleKey matches
  // this `key` exactly.
  {
    key: "DAY2",
    bonusOnly: true,
    label: "Daily Grinder",
    description: "Granted for logging in 2 days in a row.",
    color: "#f2c14e",
    gradient: "linear-gradient(120deg, #1a1200 0%, #4a3908 25%, #f2c14e 50%, #4a3908 75%, #1a1200 100%)",
  },
...
/* --------------------------------------------------------------------------
   DAILY REWARDS
   Powers the daily login reward system (components/DailyRewardsModal.jsx).
   A player can claim once per UTC calendar day. Claiming on the day right
   after their last claim advances the streak by 1; missing a day resets
   the streak back to day 1 (if resetOnMiss is true).

   - days: array of reward definitions, in order — days[0] is Day 1,
     days[1] is Day 2, etc. Add/remove/reorder freely.
   - cycleLength: once a player's streak passes the last defined day, loop
     back to Day 1 if this is set to a number (usually days.length). Set
     to null to just stay on the final day's reward forever once reached,
     instead of looping.
   - Reward `type` values (this is the ONLY thing engine.js/the API read —
     nothing else needs to change to add a new day):
       "coins" -> { amount: <number> }
       "title" -> { titleKey: <must match a TITLES[].key above,
                    typically one with bonusOnly: true> }
       "card"  -> { rarityKey: <must match a CARDS key above> }
                  (grants one random card name from that rarity)
   -------------------------------------------------------------------------- */
export const DAILY_REWARDS_CONFIG = {
  enabled: true,
  resetOnMiss: true,
  cycleLength: null, // null = stays on Day 5's reward forever once reached
  days: [
    { day: 1, type: "coins", amount: 10000 },
    { day: 2, type: "title", titleKey: "DAY2" },
    { day: 3, type: "card", rarityKey: "Digital" },
    { day: 4, type: "coins", amount: 1000000 },
    { day: 5, type: "card", rarityKey: "Veteran" },
  ],
};

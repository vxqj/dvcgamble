...
export function startPresence(onCountChange) {
  ...
  let pingTimer = null;
  let pollTimer = null;
  let rateLimitedUntil = 0; // set when ntfy 429s us; shared between ping() and pollPresence()
  let stopped = false;
  ...
  function ping() {
    seen.set(SESSION_ID, Date.now());
    if (Date.now() < rateLimitedUntil) return; // skip this tick, we're in a cooldown
    fetch(`https://ntfy.sh/${PRESENCE_CONFIG.ntfyTopic}`, {
      ...
      .then((res) => {
        if (res.status === 429) {
          ...
          rateLimitedUntil = Date.now() + (Number.isFinite(retryAfterSec) ? retryAfterSec * 1000 : 15000);
        }
      })
      .catch(() => {
  ...
  // bad time.
  // ping() and pollPresence() share `rateLimitedUntil` on purpose — a 429
  // on either one means ntfy has throttled this browser's whole shared
  // request bucket (it's per-IP, not per-topic, and the same bucket the
  // Global Feed's broadcasts draw from too), so both back off together
  // instead of one hammering away while the other cools down.

...
// isAdmin comes from the server-verified session (see lib/session.js /
// fetchMeFull) — never trust a client-side flag for this, same rule as
// every other admin-gated feature in the app.
export function unlockedTitleKeys(discoveredCards, isAdmin) {
  const have = discoveredRarityKeys(discoveredCards);
  return TITLES.filter((t) => (t.adminOnly ? !!isAdmin : have.has(t.rarityKey))).map((t) => t.key);
}

export function isTitleUnlocked(titleKey, discoveredCards, isAdmin) {

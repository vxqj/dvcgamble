...
export function fmtChance(weight, total) {
  ...
  // Below 1%, a fixed decimal count runs out of precision for very rare
  // weights and rounds them all the way down to a flat "0.00000%" — which
  // reads as "impossible" even though the weight is real and nonzero
  // (this is what happens once a rarity's weight drops much below ~0.1).
  // Instead, grow the decimal count with how small the percentage actually
  // is, so it always shows at least a couple of significant digits no
  // matter how low you push a weight.
  const magnitude = Math.floor(Math.log10(pct)); // e.g. -6 for 0.000001x%

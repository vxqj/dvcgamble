import { RARITIES, CARDS, TITLES, FEED_CONFIG, UPGRADES, COIN_PER_TICK, AUCTION_CONFIG, PACKS, PACK_SELL_REFUND_RATE, OVERCLOCKED_CONFIG } from "./config";
...
// Independent roll for the Overclocked variant — same flat-odds-per-card
// idea regardless of rarity (see OVERCLOCKED_CONFIG in config.js). Called
// once per card pulled, completely separate from the rarity roll above.
function rollOverclocked() {
  if (!OVERCLOCKED_CONFIG || !OVERCLOCKED_CONFIG.enabled) return false;
  return Math.random() < OVERCLOCKED_CONFIG.chance;
}

// One card pull: { rarity, name, isOverclocked }. luckMultiplier defaults
// to 1 (no change) so every existing call site keeps working untouched.
export function rollCard(pack, luckMultiplier = 1) {
  const rarity = rollRarity(pack, luckMultiplier);
  return { rarity, name: pickCardName(rarity), isOverclocked: rollOverclocked() };
}

// Opens a pack, returns an array of { rarity, name, isOverclocked }
// respecting the pack's guarantee rule if one is configured.
export function openPack(pack, luckMultiplier = 1) {
  ...
  if (pack.guaranteeRarity) {
    ...
    if (!hasGoodEnough && guaranteeIdx >= 0) {
      ...
      results[worstPos] = { rarity: forcedRarity, name: pickCardName(forcedRarity), isOverclocked: rollOverclocked() };
    }
  }
...
// is picked, same as a normal roll would. Forced pulls never roll
// Overclocked — they're a deliberate admin action, not a random one.
export function injectForcedCard(results, forced) {
  ...
  copy[idx] = { rarity, name, isOverclocked: false };
  return copy;
}
...
// Given a list of pull results, return the single rarest one. On a tie in
// rarity, an Overclocked copy wins — same "same rarity, Overclocked wins
// the tiebreak" rule used for the Rarest Pull Event leaderboard (see
// eventBeats below), applied locally within one pack's results.
export function bestOf(results) {
  let best = results[0];
  for (const r of results) {
    const cmpIdx = rarityIndex(r.rarity.key) - rarityIndex(best.rarity.key);
    if (cmpIdx < 0) {
      best = r;
    } else if (cmpIdx === 0 && r.isOverclocked && !best.isOverclocked) {
      best = r;
    }
  }
  return best;
...
// Coins given for selling a single card of this rarity. Pass isOverclocked
// true to apply OVERCLOCKED_CONFIG.sellMultiplier on top of the rarity's
// base sellValue, same pattern the old Rainbow variant used.
export function sellValueFor(rarityKey, isOverclocked = false) {
  const r = rarityByKey(rarityKey);
  const base = r.sellValue || 0;
  if (isOverclocked && OVERCLOCKED_CONFIG && OVERCLOCKED_CONFIG.enabled) {
    return Math.round(base * OVERCLOCKED_CONFIG.sellMultiplier);
  }
  return base;
}
...
// Coins refunded for selling ONE unopened copy of a pack, without opening
// it. Uses the pack's own `sellValue` if it defines one, otherwise falls
// back to PACK_SELL_REFUND_RATE * pack.cost.
export function packSellValueFor(pack) {
  if (!pack) return 0;
  if (typeof pack.sellValue === "number") return pack.sellValue;
  return Math.round(pack.cost * PACK_SELL_REFUND_RATE);
}

// Given packsOwned ({packKey: count}) and a set of pack keys to sell (ALL
// owned copies of each selected pack type), returns how many packs + coins
// that sell would produce, and what packsOwned looks like afterward.
// Mirrors computeSellSummary exactly, same "preview and apply share one
// source of truth" pattern.
export function computePackSellSummary(packsOwned, packKeys) {
  const keys = new Set(packKeys);
  if (keys.size === 0) return { coins: 0, count: 0, remainingPacks: packsOwned };
  let coins = 0;
  let count = 0;
  const remainingPacks = { ...packsOwned };
  PACKS.forEach((pack) => {
    const owned = packsOwned[pack.key] || 0;
    if (keys.has(pack.key) && owned > 0) {
      coins += owned * packSellValueFor(pack);
      count += owned;
      delete remainingPacks[pack.key];
    }
  });
  return { coins, count, remainingPacks };
}

// ----------------------------------------------------------------------
// Rarest Pull Event — Overclocked tiebreak helper.
// Given a NEW pull (rarityKey, isOverclocked) and the player's EXISTING
// leaderboard entry (rarityKey, isOverclocked), returns true if the new
// pull should replace the existing one. Rarity always wins first — an
// Overclocked pull can NEVER beat a genuinely rarer non-Overclocked pull.
// Overclocked only breaks a tie WITHIN the same rarity (see
// OVERCLOCKED_CONFIG.eventTiebreak in config.js). Used by
// app/api/event/submit/route.js so the "is this an improvement?" decision
// lives in one place instead of being reimplemented per-route.
// ----------------------------------------------------------------------
export function eventPullBeats(newPull, existingPull) {
  const newIdx = rarityIndex(newPull.rarityKey);
  const existingIdx = existingPull ? rarityIndex(existingPull.rarityKey) : Infinity;

  if (newIdx < existingIdx) return true; // strictly rarer, always wins
  if (newIdx > existingIdx) return false; // strictly more common, never wins

  // Same rarity — only Overclocked-vs-not matters, and only if the
  // tiebreak feature is turned on.
  if (!OVERCLOCKED_CONFIG || !OVERCLOCKED_CONFIG.eventTiebreak) return false;
  const newOC = !!newPull.isOverclocked;
  const existingOC = existingPull ? !!existingPull.isOverclocked : false;
  return newOC && !existingOC;
}

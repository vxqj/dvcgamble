...
// Was "0.00000010%" style — switched to "1/20M" style since odds on the
// rarest tiers have enough zeros that the percentage version was
// unreadable at a glance. Same signature/call shape as before (weight,
// total), so every existing caller keeps working with zero changes on
// their end — only the returned string format changed.
export function fmtChance(weight, total) {
  if (!weight || weight <= 0 || !total) return "1/∞";
  const oneIn = total / weight;
  if (oneIn <= 1) return "1/1";
  return "1/" + abbreviateOdds(oneIn);
}

// K/M/B/T/Q abbreviation for the "1/N" denominator. 2 sig figs under 10x
// the unit, 1 decimal under 100x, whole number above that — keeps
// "1/1.2M" readable without getting silly at "1/847.3M".
function abbreviateOdds(n) {
  const units = [
    { value: 1e15, suffix: "Q" },
    { value: 1e12, suffix: "T" },
    { value: 1e9, suffix: "B" },
